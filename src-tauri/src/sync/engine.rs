...
impl SyncEngine {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                if let Some(ref handle) = *app {
                    ...
                    // 推送本地变更
                    match push_changes(&conn, &cfg) {
                        Ok(count) => {
                            info!("Pushed {} changes to server", count);
                        }
                        Err(e) => {
                            warn!("Failed to push changes: {}", e);
                            {
                                let mut st = status.lock().unwrap();
                    ...
                    // 拉取远端变更
                    match pull_changes(&conn, &cfg) {
                        Ok(count) => {
                            info!("Applied {} remote changes", count);
                        }
                        Err(e) => {
                            warn!("Failed to pull changes: {}", e);
                            {
                                let mut st = status.lock().unwrap();
    ...
    pub async fn trigger_sync(&self) -> Result<(), String> {
        ...
        // 推送
        push_changes(&conn, &cfg)
            .map_err(|e| format!("推送失败: {}", e))?;

        // 拉取
        pull_changes(&conn, &cfg)
...
/// 推送本地变更到云端
fn push_changes(conn: &Connection, config: &SyncConfig) -> Result<usize, String> {
...
/// 从云端拉取变更
fn pull_changes(conn: &Connection, config: &SyncConfig) -> Result<usize, String> {
    let device_id = sync_db::get_device_id(conn)
        .map_err(|e| format!("Failed to get device_id: {}", e))?;
