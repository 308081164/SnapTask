...
pub fn register_hotkeys(app: &AppHandle) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Err(e) = app.global_shortcut().on_shortcut(area_shortcut, |_app, _shortcut, _event: ShortcutState| {
    ...
    if let Err(e) = app.global_shortcut().on_shortcut(full_shortcut, |_app, _shortcut, _event: ShortcutState| {
    ...
    if let Err(e) = app.global_shortcut().on_shortcut(window_shortcut, |_app, _shortcut, _event: ShortcutState| {
